...
/*
   ...
   x = columnPositions[i] - fborder and similar for y.
...
QTextDocumentLayout::QTextDocumentLayout(QTextDocument *doc)
    : QAbstractTextDocumentLayout(doc),
      ...
      layoutTimer(new QBasicTimer()),
      sizeChangedTimer(new QBasicTimer()),
      lastPageCount(-1)
{
    showLayoutProgress = true;
    insideDocumentChange = false;
    m_idealWidth = 0;
    m_contentHasAlignment = false;
...
{
    delete layoutTimer;
    delete sizeChangedTimer;
}

QTextFrame::Iterator QTextDocumentLayout::frameIteratorForYPosition(QFixed y) const
{
...
QTextFrame::Iterator QTextDocumentLayout::frameIteratorForTextPosition(int position) const
{
    QTextFrame *rootFrame = document()->rootFrame();
    ...
    // If the position is on a child frame
    // the iterator should point to the direct child frame of the root frame
    QTextFrame *containingFrame = document()->frameAt(blockPos);

    if (containingFrame && containingFrame != rootFrame) {
        while (containingFrame->parentFrame()
               && containingFrame->parentFrame() != rootFrame) {
            containingFrame = containingFrame->parentFrame();
        }

        for (QTextFrame::Iterator it = rootFrame->begin();
             !it.atEnd();
             ++it) {
            if (it.currentFrame() == containingFrame)
                return it;
        }
    ...
    // default case: position is in a direct text block of the root frame
    for (QTextFrame::Iterator it = rootFrame->begin();
         !it.atEnd();
         ++it) {
        QTextBlock currentBlock = it.currentBlock();
        ...
        // If operator== doesn't work, alternatively:
        // if (currentBlock.position() == block.position())
        //     return it;
    }
...
QTextDocumentLayout::HitPoint
QTextDocumentLayout::hitTest(QTextFrame *frame, const QFixedPoint &point, int *position, QTextLayout **l, Qt::HitTestAccuracy accuracy) const
{
...
QTextDocumentLayout::HitPoint
QTextDocumentLayout::hitTest(QTextFrame::Iterator it, HitPoint hit, const QFixedPoint &p,
...
QTextDocumentLayout::HitPoint
QTextDocumentLayout::hitTest(QTextTable *table, const QFixedPoint &point,
                                    int *position, QTextLayout **l, Qt::HitTestAccuracy accuracy) const
{
    QTextTableData *td = static_cast<QTextTableData *>(data(table));
...
QTextDocumentLayout::HitPoint
QTextDocumentLayout::hitTest(const QTextBlock &bl, const QFixedPoint &point, int *position, QTextLayout **l,
...
QFixed QTextDocumentLayout::blockIndent(const QTextBlockFormat &blockFormat) const
{
...
void QTextDocumentLayout::drawBorder(QPainter *painter, const QRectF &rect, qreal topMargin, qreal bottomMargin,
...
void QTextDocumentLayout::drawFrameDecoration(QPainter *painter, QTextFrame *frame, QTextFrameData *fd, const QRectF &clip, const QRectF &rect) const
{
...
static inline qreal collapseOffset(const QTextDocumentLayout *p, const EdgeData &w)
{
...
static inline double prioritizedEdgeAnchorOffset(const QTextDocumentLayout *p,
...
static inline
void drawCellBorder(const QTextDocumentLayout *p, QPainter *painter,
...
void QTextDocumentLayout::drawTableCellBorder(const QRectF &cellRect, QPainter *painter,
...
void QTextDocumentLayout::drawTableCell(const QRectF &cellRect, QPainter *painter, const QAbstractTextDocumentLayout::PaintContext &cell_context,
...
void QTextDocumentLayout::drawFlow(const QPointF &offset, QPainter *painter, const QAbstractTextDocumentLayout::PaintContext &context,
                                   QTextFrame::Iterator it, const QList<QTextFrame *> &floats, QTextBlock *cursorBlockNeedingRepaint) const
{
    const QTextDocumentLayout *q = this;
...
void QTextDocumentLayout::drawBlock(const QPointF &offset, QPainter *painter,
                                    const QAbstractTextDocumentLayout::PaintContext &context,
                                    const QTextBlock &bl, bool inRootFrame) const
{
    ...
    if (!isEmptyBlockBeforeTable(frameIteratorForTextPosition(blpos))
        ...
        if (cpos < -1)
            ...
            cpos -= blpos;
        tl->drawCursor(painter, offset, cpos, m_cursorWidth);
    }
...
void QTextDocumentLayout::drawListItem(const QPointF &offset, QPainter *painter,
                                       const QAbstractTextDocumentLayout::PaintContext &context,
                                       const QTextBlock &bl, const QTextCharFormat *selectionFormat) const
{
    QTextDocumentLayout *q = const_cast<QTextDocumentLayout *>(this);
...
QTextLayoutStruct QTextDocumentLayout::layoutCell(QTextTable *t, const QTextTableCell &cell, QFixed width,
...
QRectF QTextDocumentLayout::layoutTable(QTextTable *table, int layoutFrom, int layoutTo, QFixed parentY)
{
    ...
    const QTextOption::WrapMode oldDefaultWrapMode = document()->defaultTextOption().wrapMode();
...
recalc_minmax_widths:
    ...
    // two cells is exactly 2 * tcellBorder pixel. we do this by rounding the calculated width
    // values here.
    ...
    // mode
    if (document()->defaultTextOption().wrapMode() == QTextOption::WrapAtWordBoundaryOrAnywhere
        && contentsWidth > td->contentsWidth) {
        document()->defaultTextOption().setWrapMode(QTextOption::WrapAnywhere);
    ...
    document()->defaultTextOption().setWrapMode(oldDefaultWrapMode);
...
void QTextDocumentLayout::positionFloat(QTextFrame *frame, QTextLine *currentLine)
{
    QTextFrameData *fd = data(frame);
...
//     qDebug() << "positionFloat:" << frame << "width=" << fsize.width;
...
//     qDebug()<< "float positioned at " << fposition.x << fposition.y;
...
QRectF QTextDocumentLayout::layoutFrame(QTextFrame *f, int layoutFrom, int layoutTo, QFixed parentY)
{
...
QRectF QTextDocumentLayout::layoutFrame(QTextFrame *f, int layoutFrom, int layoutTo, QFixed frameWidth, QFixed frameHeight, QFixed parentY)
{
    qCDebug(lcLayout, "layoutFrame (%d--%d), parent=%p", f->firstPosition(), f->lastPosition(), f->parentFrame());
    Q_ASSERT(data(f)->sizeDirty);
    ...
    // set fcontentsWidth temporarily, so that layoutFrame for the children
...
void QTextDocumentLayout::layoutFlow(QTextFrame::Iterator it, QTextLayoutStruct *layoutStruct,
...
void QTextDocumentLayout::layoutBlock(const QTextBlock &bl, int blockPosition, const QTextBlockFormat &blockFormat,
                                      QTextLayoutStruct *layoutStruct, int layoutFrom, int layoutTo, const QTextBlockFormat *previousBlockFormat)
{
    QTextDocumentLayout *q = this;
    ...
    if (layoutStruct->fullLayout
        ...
        option.setAlignment(QStyle::visualAlignment(dir, align));
...
void QTextDocumentLayout::floatMargins(QFixed y, const QTextLayoutStruct *layoutStruct,
...
//                 qDebug() << "adjusting with float" << f << fposition.x()<< fsize.width();
...
QFixed QTextDocumentLayout::findY(QFixed yFrom, const QTextLayoutStruct *layoutStruct, QFixed requiredWidth) const
{
    QFixed right, left;
...
void QTextDocumentLayout::draw(QPainter *painter, const PaintContext &context)
{
    ...
    if (document()->pageSize().width() == 0 && viewportRect.isValid()) {
...
void QTextDocumentLayout::documentChanged(int from, int oldLength, int length)
{
    ...
    if (!(document()->isLayoutEnabled() && !document()->pageSize().isNull()))
        return;
    ...
    if (smallChange
        && (currentLazyLayoutPosition == -1 || showLayoutProgress == false))
...
QRectF QTextDocumentLayout::doLayout(int from, int oldLength, int length)
{
    ...
    if (data(root)->sizeDirty)
        updateRect = layoutFrame(root, from, from + length);
...
{
    ensureLayouted(QFixed::fromReal(point.y()));
    ...
    QTextDocumentLayout::HitPoint p = hitTest(f, pointf, &position, &l, accuracy);
    if (accuracy == Qt::ExactHit && p < QTextDocumentLayout::PointExact)
        return -1;
...
void QTextDocumentLayout::positionInlineObject(QTextInlineObject item, int posInDocument, const QTextFormat &format)
{
    ...
    QTextCharFormat f = format.toCharFormat();
    Q_ASSERT(f.isValid());
    if (!handlerForObject(f.objectType()))
        return;
...
//         frame->firstPosition() << frame->lastPosition();
    positionFloat(frame, line.isValid() ? &line : nullptr);
}
...
void QTextDocumentLayout::ensureLayouted(QFixed y) const
{
    const QTextDocumentLayout *q = this;
...
void QTextDocumentLayout::ensureLayoutedByPosition(int position) const
{
    if (currentLazyLayoutPosition == -1)
        return;
    ...
    while (currentLazyLayoutPosition != -1
        ...
        const_cast<QTextDocumentLayout *>(this)->doLayout(currentLazyLayoutPosition, 0, INT_MAX - currentLazyLayoutPosition);
    }
}

void QTextDocumentLayout::layoutStep() const
{
...
QRectF QTextDocumentLayout::frameBoundingRect(QTextFrame *frame) const
{
    if (!canLayout())
        return QRectF();
    ensureLayoutFinished();
    return frameBoundingRectInternal(frame);
}

QRectF QTextDocumentLayout::frameBoundingRectInternal(QTextFrame *frame) const
{
    QPointF pos;
...
{
    if (!canLayout() || !block.isValid() || !block.isVisible())
        return QRectF();
...
{
    int pos = currentLazyLayoutPosition;
    if (pos == -1)
        return 100;
...
{
    ensureLayouted(QFixed::fromReal(y));
}
...
{
    return m_contentHasAlignment;
}

qreal QTextDocumentLayout::scaleToDevice(qreal value) const
{
    if (!paintDevice())
        return value;
...
QFixed QTextDocumentLayout::scaleToDevice(QFixed value) const
{
    if (!paintDevice())
        return value;
